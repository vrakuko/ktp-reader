...
from validrombak import validagama, validgender, validkab, validkawin, validkec, validkeldes, validkota, validnama, validnik, validprov, validrtrw, validdarah, validjob, validwn, validberlaku, validgeoprov, validgeokec, is_mostly_digits
ktpdata_global = {}
...
def straighten_ktp(image_bytes: bytes, save_path: str = None, 
    ...
    Termasuk penanganan rotasi 90/180/270 derajat menggunakan Tesseract OSD.
    ...
    # --- LANGKAH 1: Deteksi dan Koreksi Orientasi Mayor (0, 90, 180, 270 derajat) ---
...
def extrocr(ocrdict:dict) -> dict:
    ...
    for i, word in enumerate(arrtxtocr):
        ...
        if i==0 : #or kata == "provinsi" :
            ...
            if  is_mostly_digits(arrtxtocr[i+2]) or is_mostly_digits(arrtxtocr[i+3]):
                val = validgeoprov(ktpdata['provinsi'], word)
                if 'Kota' in val:
        ...
        elif (i ==6 and is_mostly_digits(arrtxtocr[i-3]) ) or (i==7 and is_mostly_digits(arrtxtocr[i-4])) or (i==6 and countvalnama ==1) or (i==7 and countvalnama==2) or (kata == 'tempat tgl lahir' and i<10)  :        #and (  (i-arrktpdata[arrktpdata.index('nama')]) ==2 )
            ...
            carryocr.remove(arrtxtocr[i+1])
            j = i+2
            if is_mostly_digits(arrtxtocr[j]):
                ...
                j+=1
                i+=1
            # while (process.extractOne(arrtxtocr[j], ['LAKI-LAKI', 'PEREMPUAN'])[1]<21):


            # if process.extractOne(arrtxtocr[i+2], ktp_fields)[0] != 'tempat tgl lahir':
            #     namaval.join(f" {arrtxtocr[i+2]}")
        ...
        elif (i ==8 and countvalttl==1 and countvalnama == 1) or (i ==9 and (countvalttl==2 or countvalnama==2 ))  or (kata=='jenis kelamin' and i<10) :         #and (  (i-arrktpdata[arrktpdata.index('tempat tgl lahir')]) ==6 or (i-arrktpdata[arrktpdata.index('tempat tgl lahir')])==3  ) 
        ...
        elif (i==10 and countvalnama==1 ) or (i==11 and (countvalnama==2 or countvaladr==2)) or (kata == 'gol darah' and i>8 and i<15) :           #and  (i-arrktpdata[arrktpdata.index('jenis kelamin')]) ==2) 
    #cek apakah setelah kata 'golongan darah' apakah field atau engga (field alamat)
        ...
        elif (i==11 and  ktpdata['gol darah']=='dummy') or (i==12 and  (ktpdata['gol darah']!='dummy' or countvalnama == 2 or countvaladr==2)) or  (kata == 'alamat' and i>8 and i<15)   :
                            # if i>=8 and i < 20:
            ...
            carryocr.remove(arrtxtocr[i+1])
            j = i+2
            if process.extractOne(arrtxtocr[j], ktp_fields)[0] not in ['rt rw', 'kewarganegaraan', 'status perkawinan']:
        ...
        elif (i == 13 ) or(i==14 and (countvalnama == 2 or countvaladr==2) )  or (kata == 'rt rw' and i>8 and i<17) :          # and process.extractOne(arrtxtocr[i+2], ktp_fields)[0] in ['kecamatan', 'kel desa']
            # if  i>10 and i<20:
            # rtrwarr = []
            # j = i + 1  # mulai cek setelah field "nama"
        ...
        elif (i == 15 )or (i==16 and (countvalnama == 2 or countvaladr==2)) or (kata == 'kel desa' and i>12 and i<20)  :          #and 'alamat' in ktpdata
            if i > 10 and i < 20:
                nextword = arrtxtocr[i+1] if i+1 < len(arrtxtocr) else None
                listnamakel = list(kel_df['name'].values)
                kelval = validkeldes(nextword, listnamakel)
        ...
        elif i==17 or (i==18 and (countvalnama == 2 or countvaladr==2)) or (kata == 'kecamatan' and process.extractOne(arrtxtocr[i+2], ktp_fields)[0] in ['agama', 'nama']) :        #and 'alamat' in ktpdata) or (process.extractOne(arrtxtocr[i+2], ktp_fields)[0] in ['agama', 'nama']

            nextword = arrtxtocr[i+1] if i+1 < len(arrtxtocr) and len(word)<=12 else None
            # yetword = arrtxtocr[i-1]
            kecval = validkec(word,  nextword)
            if not nextword:
                val = validgeokec(word, ktpdata['kel desa'])
            else :
                val = validgeokec(nextword,ktpdata['kel desa'] )

            arrktpdata[arrktpdata.index('kel desa')+1] = val 
            ktpdata['kel desa'] = val
        ...
        elif i==19 or (i==20 and (countvalnama == 2 or countvaladr==2)) or (kata == 'agama' and i<24 and i >15):
            nextword = arrtxtocr[i+1] if i+1 < len(arrtxtocr) else None
            ...
            nextword = arrtxtocr[i+1] if i+1 < len(arrtxtocr) and len(word) < 18 else None
        ...
        elif (i==24 and 'nyatu' in ktpdata['status perkawinan']) or (i==25 and 'misah' in ktpdata['status perkawinan']) or (kata == 'kewarganegaraan' and i>20):
            ...
            nextword = arrtxtocr[i+1] if i+1 < len(arrtxtocr) and len(word) <17 else None
            ...
            arrktpdata[arrktpdata.index('kewarganegaraan')+1] = wnval   # ['wninsi', 'jawa timur']
            ktpdata['kewarganegaraan'] = wnval                           # {'wninsi' : 'jawa timur' }
            wordmatched.add(i)                                  
            carryocr.remove(word) 
            print(f"data kewarganegaraan : {wnval} sukses ditambahkan")   
            # else:
            #     print(f'kata utk {word} seharusnya bukan {kata}, skor = {score}')
...
datadir = 'data'
imgpath = os.path.join(datadir, 'ktp57.jpg')
...
rotated_img, angle = straighten_ktp(image_bytes_from_file, save_path="ktp57_lurus.jpg", perform_osd=True, osd_pagesegmode='0' )
...
if 'image_with_boxes_bytes' in ocr_results and ocr_results['image_with_boxes_bytes'] is not None:
    output_image_path = f'ktp57_boxes.jpg'
