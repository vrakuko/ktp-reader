...
def is_mostly_digits(text: str) -> bool:
    """
    Fungsi untuk mengecek apakah sebuah string lebih dari separuhnya adalah angka.
    Contoh:
    - is_mostly_digits("20-12-2025") -> True
    - is_mostly_digits("SEUMUR H1DUP") -> False
    - is_mostly_digits("SEUMURHIDUP") -> False
    """
    # Menangani jika input bukan string atau string kosong
    if not isinstance(text, str) or not text:
        return False

    # Menghitung jumlah karakter yang merupakan angka
    # sum() akan menganggap True sebagai 1 dan False sebagai 0
...
#     return kelval, kelid, carrykel



# # def validalamat (word: str):
# def validkec  (word : str, wordyet = str , nextword : str= None) -> str:
#     listnamakec = list(kec_df['name'].values)
#     if len(word) > 12 :
#         kecval, _ = process.extractOne(word, listnamakec)
#         kecval += ' nyatu'
...
#             if match :
#                 kec_dict = kec_df.set_index('name')['id'].astype(str).to_dict()
#                 kecid = kec_dict[match]

#                 kelname, kelid, carrykel = validkeldes(wordyet)
                
#                 if kelid :
#                     if kecid in kelid :
#                         kecval = match  + ' misah'
#                     else :
#                         while kecid not in kelid:
#                             carrykel.remove(kelname)
#                             kelname, kelid, carrykel = validkeldes(wordyet)
